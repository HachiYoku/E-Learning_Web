...
const getActionableSupportCount = async (_req, res) => {
  try {
    const tickets = await SupportTicket.find({ status: { $ne: "resolved" } }).select("replies").lean();
    const count = tickets.filter((ticket) => !ticket.replies.length || ticket.replies.at(-1).authorRole === "user").length;
...
module.exports = { createSupportTicket, getMySupportTickets, getSupportTickets, getActionableSupportCount, updateTicketStatus, replyToSupportTicket, replyToOwnSupportTicket };
