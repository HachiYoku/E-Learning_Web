...
function AddBlog() {
  ...
  return (
    <div className="min-h-screen bg-[#FFFDF8]">
      <div className="border-b border-[#E58C1A]/15 bg-[#FFFDF8] px-4 py-4 sm:px-6 md:px-8">
        <div className="mx-auto flex max-w-7xl flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <div className="flex-1 sm:text-center"><p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#C97112]">Content library</p><h1 className="mt-1 text-left text-2xl font-bold tracking-tight text-[#2D2E30] sm:text-center sm:text-3xl">Create blog post</h1></div>
          <div className="flex w-full items-center gap-2 sm:w-auto">
            <button
              onClick={handleSubmit}
              disabled={isSubmitting}
              className="flex-1 rounded-xl bg-[#2D2E30] px-4 py-2.5 text-sm font-bold text-white shadow-md shadow-[#2D2E30]/15 transition-colors hover:bg-[#E58C1A] disabled:cursor-not-allowed disabled:opacity-70 sm:flex-none sm:px-6"
            >
      ...
      <>
        ...
        >
          ...
          <div className="flex h-9 items-center rounded border border-gray-200 bg-white" aria-label="Text size">
            <button type="button" onMouseDown={(event) => { event.preventDefault(); saveEditorSelection(); }} onClick={() => applyFontSize(Math.max(fontSize - 1, 8))} className="px-2 text-base font-semibold text-gray-500 transition-colors hover:text-pink-600" data-tooltip="Decrease font size" aria-label="Decrease font size">-</button>
            <input
              type="number"
              min="8"
              max="72"
            ...
            />
            <button type="button" onMouseDown={(event) => { event.preventDefault(); saveEditorSelection(); }} onClick={() => applyFontSize(Math.min(fontSize + 1, 72))} className="px-2 text-base font-semibold text-gray-500 transition-colors hover:text-pink-600" data-tooltip="Increase font size" aria-label="Increase font size">+</button>
          </div>
      ...
      {isDiscardDialogOpen ? (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl"><h2 className="text-lg font-bold text-[#2D2E30]">Discard unsaved changes?</h2>
            <p className="mt-2 text-sm text-[#765F55]">Your blog draft has changes that have not been published. If you leave now, they will be lost.</p>
