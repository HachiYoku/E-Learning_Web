...
function QuizEditor() {
  ...
  const [form, setForm] = useState({ courseId: "", lessonId: "", quizType: "lesson", title: "", maxAttempts: "", questions: [newQuestion()] });
  ...
  const submit = async (event) => {
    ...
    if (questions.some((q) => !(q.image || q.imageFile) || q.options.filter(Boolean).length < 2 || q.correctAnswer >= q.options.filter(Boolean).length)) return setError("Each question needs an image, at least two answers, and a valid correct answer.");
    try {
