...
function Quizzes() {
  ...
  return <div className="min-h-screen bg-gray-50 p-4 sm:p-6 md:p-8">
    ...
    {loading ? <p className="text-gray-600">Loading quizzes...</p> : quizzes.length === 0 ? <div className="rounded-xl bg-white p-10 text-center shadow"><p className="text-gray-600">No quizzes yet. Create your first lesson quiz.</p></div> : <div className="grid gap-4 lg:grid-cols-2">
      {quizzes.map((quiz) => <article key={quiz.id} className="rounded-xl bg-white p-5 shadow-sm ring-1 ring-gray-100">
        <div className="flex items-start justify-between gap-3"><div><h2 className="text-xl font-bold text-gray-900">{quiz.title}</h2><p className="mt-2 text-sm text-gray-600">{quiz.course?.title || "Course"} · Lesson {quiz.lesson?.order || ""}: {quiz.lesson?.title || "Lesson"}</p><p className="mt-2 text-sm font-medium text-pink-600">{quiz.questions.length} question{quiz.questions.length === 1 ? "" : "s"}</p></div>
