...
function Flashcards() {
  ...
  return <Page onClickCapture={requestDiscard}>
    ...
    {categoryCards.length ? <div className="grid gap-4 md:grid-cols-2 md:gap-5">{categoryCards.map((card, index) => <article key={card.id} className="group flex h-full flex-col overflow-hidden rounded-3xl border border-[#2D2E30]/10 bg-white shadow-[0_16px_38px_-30px_rgba(45,46,48,.7)] transition duration-200 hover:-translate-y-1 hover:border-[#E58C1A]/45 hover:shadow-[0_22px_44px_-28px_rgba(201,113,18,.35)]"><div className="h-1.5 bg-gradient-to-r from-[#E58C1A] via-[#F4B343] to-[#FFF1CE]" /><div className="flex flex-1 flex-col gap-4 p-4 sm:flex-row sm:gap-5 sm:p-5"><div className="relative h-36 w-full shrink-0 overflow-hidden rounded-2xl border border-[#E58C1A]/20 bg-gradient-to-br from-[#FFF9EA] via-[#FFF1CE] to-[#F7C66B] sm:h-auto sm:min-h-36 sm:w-32">{card.image ? <img src={card.image} alt="" className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center text-4xl">📝</div>}<span className="absolute bottom-2 left-2 rounded-full bg-[#2D2E30]/80 px-2 py-1 text-[9px] font-bold uppercase tracking-[.14em] text-white">Card {String(index + 1).padStart(2, "0")}</span></div><div className="min-w-0 flex-1 space-y-3"><section className="rounded-2xl border border-[#2D2E30]/8 bg-[#FFFDF8] px-4 py-3.5"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#9B867C]">Front · prompt</p><p className="mt-1.5 break-words text-[17px] font-bold leading-6 text-[#2D2E30]">{card.prompt}</p></section><section className="rounded-2xl border border-[#E58C1A]/20 bg-gradient-to-br from-[#FFFDF8] to-[#FFF4DB] px-4 py-3.5"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#9A5816]">Back · answer</p><p className="mt-1.5 break-words text-[17px] font-bold leading-6 text-[#C97112]">{card.answer}</p>{card.translation && <p className="mt-2 border-t border-[#E58C1A]/15 pt-2 break-words text-sm leading-6 text-[#765F55]">{card.translation}</p>}</section></div></div><div className="flex flex-col gap-3 border-t border-[#2D2E30]/10 bg-[#FFFDF8] px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between"><span className={`w-fit rounded-full px-2.5 py-1 text-xs font-bold ${card.isPublished ? "bg-[#E9F4EA] text-[#246B35]" : "bg-[#F2EFEB] text-[#765F55]"}`}>{card.isPublished ? "Published" : "Draft"}</span><div className="grid w-full grid-cols-2 gap-2 sm:w-auto"><button onClick={() => { setError(""); setForm({ ...card, category: id(category), imageFile: null, removeImage: false }); }} className="button-secondary w-full px-2.5 sm:px-4"><Edit3 size={15} /> Edit</button><button onClick={() => setDeleting({ type: "card", item: card })} className="button-danger w-full px-2.5 sm:px-4"><Trash2 size={15} /> Delete</button></div></div></article>)}</div> : <Empty message="No flashcards in this category yet. Add the first one to get started." />}
