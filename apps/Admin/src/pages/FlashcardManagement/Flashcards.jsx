...
function Flashcards() {
  ...
  if (!category) return <Page onClickCapture={requestDiscard}>
    <Header title="Flashcard categories" text="Create clear practice sets for your learners." actions={<button onClick={() => openCategoryModal()} className="button-primary w-full px-5 py-3 sm:w-auto"><Plus size={18} /> Create category</button>} />
    {error && <Error error={error} />}
    {categories.length ? <div className="grid gap-4 sm:grid-cols-2 sm:gap-5 xl:grid-cols-3">{categories.map((item) => <article key={id(item)} className="overflow-hidden rounded-3xl border border-[#2D2E30]/10 bg-white p-4 shadow-[0_14px_34px_-28px_rgba(45,46,48,.6)] transition duration-200 hover:-translate-y-1 hover:border-[#E58C1A]/40 hover:shadow-lg sm:p-5"><button onClick={() => { setCategory(item); setError(""); }} className="w-full text-left"><div className="flex items-start justify-between gap-3"><span className="rounded-2xl bg-[#FFF1CE] p-3 text-[#C97112]"><FolderOpen size={22} /></span><span className="shrink-0 rounded-full bg-[#FFF9EA] px-2.5 py-1 text-xs font-bold text-[#765F55]">{item.cardCount || 0} cards</span></div><h2 className="mt-5 break-words text-lg font-bold text-[#2D2E30] sm:mt-7 sm:text-xl">{item.name}</h2><p className="mt-2 text-sm leading-6 text-[#765F55]">Open and manage this set <span className="font-bold text-[#C97112]">→</span></p></button><div className="mt-5 grid grid-cols-2 gap-2 border-t border-[#2D2E30]/10 pt-4"><button type="button" onClick={() => openCategoryModal(item)} className="button-secondary min-w-0 justify-center px-2 sm:px-4"><Pencil size={16} /> Edit</button><button type="button" disabled={Boolean(item.cardCount)} title={item.cardCount ? "Remove all cards before deleting this category" : "Delete category"} onClick={() => setDeleting({ type: "category", item })} className="button-danger min-w-0 justify-center px-2 sm:px-4 disabled:cursor-not-allowed disabled:opacity-45"><Trash2 size={16} /> Delete</button></div></article>)}</div> : <Empty />}
    ...
    <ConfirmationModal isOpen={Boolean(deleting)} title="Delete category" message={`Delete “${deleting?.item?.name || ""}”? This action cannot be undone.`} confirmText="Delete" cancelText="Cancel" onConfirm={confirmDelete} onCancel={() => setDeleting(null)} isDangerous />
  ...
  return <Page onClickCapture={requestDiscard}>
    ...
    <Header title={category.name} text={`${categoryCards.length} cards in this category.`} actions={<button onClick={() => { setError(""); setForm(blankCard(category)); }} className="button-primary w-full sm:w-auto"><Plus size={18} /> Add flashcard</button>} />
    {error && <Error error={error} />}
    <div className="mb-6 rounded-2xl border border-[#E58C1A]/15 bg-[#FFF9EA] px-4 py-3 text-sm text-[#765F55]">New and edited cards stay in <strong className="text-[#2D2E30]">{category.name}</strong> to keep this practice set organized.</div>
    {categoryCards.length ? <div className="grid gap-4 md:grid-cols-2 md:gap-5">{categoryCards.map((card, index) => <article key={card.id} className="group flex h-full flex-col overflow-hidden rounded-2xl border border-[#2D2E30]/10 bg-white shadow-[0_14px_32px_-28px_rgba(45,46,48,.48)] transition duration-200 hover:-translate-y-0.5 hover:border-[#E58C1A]/40 hover:shadow-[0_20px_38px_-28px_rgba(201,113,18,.28)]"><div className="flex flex-1 flex-col gap-4 p-4 sm:flex-row sm:gap-5 sm:p-5"><div className="relative h-36 w-full shrink-0 overflow-hidden rounded-xl border border-[#E58C1A]/20 bg-[#FFF9EA] sm:h-auto sm:min-h-36 sm:w-32">{card.image ? <img src={card.image} alt="" className="h-full w-full object-cover transition duration-300 group-hover:scale-105" /> : <div className="flex h-full items-center justify-center text-4xl">📝</div>}<span className="absolute left-2 top-2 rounded-lg bg-white/90 px-2 py-1 text-[9px] font-bold uppercase tracking-[.14em] text-[#765F55] shadow-sm">{String(index + 1).padStart(2, "0")}</span></div><div className="min-w-0 flex-1 space-y-3"><section className="rounded-xl border border-[#2D2E30]/10 bg-[#FFFDF8] px-4 py-3.5"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#9B867C]">Front · prompt</p><p className="mt-1.5 break-words text-[17px] font-bold leading-6 text-[#2D2E30]">{card.prompt}</p></section><section className="rounded-xl border border-[#E58C1A]/18 bg-[#FFF9EA] px-4 py-3.5"><p className="text-[10px] font-bold uppercase tracking-[.16em] text-[#9A5816]">Back · answer</p><p className="mt-1.5 break-words text-[17px] font-bold leading-6 text-[#C97112]">{card.answer}</p>{card.translation && <p className="mt-2 border-t border-[#E58C1A]/15 pt-2 break-words text-sm leading-6 text-[#765F55]">{card.translation}</p>}</section></div></div><div className="flex flex-col gap-3 border-t border-[#2D2E30]/10 px-4 py-3.5 sm:flex-row sm:items-center sm:justify-between"><span className={`w-fit rounded-full px-2.5 py-1 text-xs font-bold ${card.isPublished ? "bg-[#E9F4EA] text-[#246B35]" : "bg-[#F2EFEB] text-[#765F55]"}`}>{card.isPublished ? "Published" : "Draft"}</span><div className="grid w-full grid-cols-2 gap-2 sm:w-auto"><button onClick={() => { setError(""); setForm({ ...card, category: id(category), imageFile: null, removeImage: false }); }} className="button-secondary w-full px-2.5 sm:px-4"><Edit3 size={15} /> Edit</button><button onClick={() => setDeleting({ type: "card", item: card })} className="button-danger w-full px-2.5 sm:px-4"><Trash2 size={15} /> Delete</button></div></div></article>)}</div> : <Empty message="No flashcards in this category yet. Add the first one to get started." />}
...
function Header({ title, text, actions }) { return <header className="mb-7 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div className="min-w-0"><p className="text-[10px] font-bold uppercase tracking-[.2em] text-[#C97112]">Practice content</p><h1 className="mt-2 break-words text-3xl font-bold tracking-tight text-[#2D2E30] sm:text-4xl">{title}</h1><p className="mt-2 text-[#765F55]">{text}</p></div>{actions && <div className="flex w-full flex-wrap gap-2 sm:w-auto">{actions}</div>}</header>; }
