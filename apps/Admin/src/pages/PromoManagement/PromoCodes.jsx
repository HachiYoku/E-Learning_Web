...
function PasswordModal({ title, description, password, error, saving, onChange, onConfirm, onClose, dangerous = false }) {
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#2D2E30]/60 p-4 backdrop-blur-[2px]"><div className="w-full max-w-md overflow-hidden rounded-2xl bg-white shadow-2xl"><div className={`flex items-center justify-between border-b p-5 ${dangerous ? "border-[#A34D45]/15 bg-[#FFF0EE]" : "border-[#E58C1A]/15 bg-[#FFF9EA]"}`}><div><p className={`text-[10px] font-bold uppercase tracking-[.18em] ${dangerous ? "text-[#A34D45]" : "text-[#C97112]"}`}>Protected action</p><h2 className="mt-1 text-lg font-bold">{title}</h2></div><button onClick={onClose} disabled={saving} className="rounded-lg p-1 hover:bg-white" aria-label="Close confirmation"><X size={20} /></button></div><div className="p-5"><p className="mb-4 text-sm leading-6 text-[#765F55]">{description}</p>{error && <div className="mb-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2.5 text-sm text-red-700">{error}</div>}<label className="mb-2 block text-sm font-bold">Confirm admin password</label><input autoFocus type="password" value={password} onChange={(e) => onChange(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter") onConfirm(); }} placeholder="Enter your admin password to confirm" className="w-full rounded-xl border border-[#2D2E30]/15 px-3 py-3 text-sm outline-none focus:border-[#E58C1A] focus:ring-4 focus:ring-[#E58C1A]/10" disabled={saving} autoComplete="current-password" /></div><div className="flex gap-3 border-t border-[#2D2E30]/10 bg-[#FFFDF8] p-5"><button onClick={onClose} disabled={saving} className="flex-1 rounded-xl border border-[#2D2E30]/15 bg-white px-4 py-2.5 font-semibold hover:bg-[#FFF4D8]">Cancel</button><button onClick={onConfirm} disabled={saving} className={`flex-1 rounded-xl px-4 py-2.5 font-bold text-white disabled:opacity-60 ${dangerous ? "bg-[#A34D45] hover:bg-[#8D4039]" : "bg-[#2D2E30] hover:bg-[#E58C1A]"}`}>{saving ? "Working..." : title}</button></div></div></div>;
}
