...
import { buildPromoPayload } from "./promoFormState";

const blank = () => ({ code: "", discountType: "percent", discountValue: "", fixedAmounts: { THB: "", MMK: "" }, usageLimit: "", startsAt: "", expiresAt: "", applicableCourses: [], isActive: true });
...
function PromoCodes() {
  ...
  const payload = () => ({ ...buildPromoPayload(form), usageLimit: form.usageLimit ? Number(form.usageLimit) : null, startsAt: used ? form._startsAtRaw : (form.startsAt || null), expiresAt: used ? form._expiresAtRaw : (form.expiresAt || null), applicableCourses: form.applicableCourses || [] });
  const openEdit = (promo) => setForm({ ...promo, fixedAmounts: { THB: promo.fixedAmounts?.THB ?? "", MMK: promo.fixedAmounts?.MMK ?? "" }, _startsAtRaw: promo.startsAt || null, _expiresAtRaw: promo.expiresAt || null, startsAt: isoDate(promo.startsAt), expiresAt: isoDate(promo.expiresAt), usageLimit: promo.usageLimit || "", applicableCourses: (promo.applicableCourses || []).map((course) => String(course._id || course)) });
  ...
  return <div className="min-h-screen bg-[#FFFDF8] p-4 sm:p-6 md:p-8"><div className="mx-auto max-w-6xl">
    ...
    <div className="overflow-hidden rounded-2xl border border-[#2D2E30]/10 bg-white">{codes.length ? codes.map((code) => { const status = getPromoStatus(code); const scope = code.applicableCourses?.length ? code.applicableCourses.map((course) => course.title || "Selected course").join(", ") : "All courses"; const offer = code.discountType === "percent" ? `${code.discountValue}% off · Automatically applies in THB and MMK` : Object.entries(code.fixedAmounts || {}).map(([currency, amount]) => `${currency === "THB" ? "฿" : "Ks "}${Number(amount).toLocaleString()} off`).join(" · ") || `Legacy THB ฿${Number(code.discountValue).toLocaleString()} off`; return <div key={code._id} className="flex flex-col gap-3 border-b border-[#2D2E30]/10 p-4 last:border-0 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex flex-wrap items-center gap-2"><code className="rounded-lg bg-[#FFF1CE] px-2 py-1 font-bold text-[#9A5816]">{code.code}</code><span className={`rounded-full px-2 py-1 text-xs font-bold ${status.className}`}>{status.label}</span>{code.hasHistory && <span className="rounded-full bg-[#F3E9D9] px-2 py-1 text-xs font-bold text-[#765F55]">Used history</span>}</div><p className="mt-2 text-sm text-[#765F55]">{offer} · Usage {code.usageCount}{code.usageLimit ? ` / ${code.usageLimit}` : " / Unlimited"} · {scope}</p><p className="mt-1 text-xs text-[#9A8775]">{code.startsAt ? `Starts ${new Date(code.startsAt).toLocaleDateString()} · ` : "Starts immediately · "}{code.expiresAt ? `Ends ${new Date(code.expiresAt).toLocaleDateString()}` : "No expiry"}</p></div><div className="flex gap-2"><button onClick={() => openEdit(code)} className="rounded-lg px-3 py-2 text-sm font-bold hover:bg-[#FFF4D8]">Edit</button><button onClick={() => { setDeleteTarget(code); setDeletePassword(""); setDeleteError(""); }} className="inline-flex items-center gap-1 rounded-lg px-3 py-2 text-sm font-bold text-[#A34D45] hover:bg-[#FFF0EE]">{code.hasHistory ? <Archive size={15} /> : <Trash2 size={15} />}{code.hasHistory ? "Archive" : "Delete"}</button></div></div> }) : <div className="p-10 text-center text-[#765F55]">No promo codes yet.</div>}</div>
...
function PasswordModal({ title, description, password, error, saving, onChange, onConfirm, onClose, dangerous = false, confirmLabel = "Confirm and create" }) {
  const panel = dangerous ? "border-[#A34D45]/15" : "border-[#2D2E30]/10";
  const header = dangerous ? "border-[#A34D45]/15 bg-[#FFF0EE]" : "border-[#E58C1A]/15 bg-[#FFF9EA]";
  const eyebrow = dangerous ? "text-[#A34D45]" : "text-[#C97112]";
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-[#2D2E30]/60 p-4 backdrop-blur-[2px]">
    <div className={`w-full max-w-md overflow-hidden rounded-2xl border bg-white shadow-2xl shadow-[#2D2E30]/25 ${panel}`} role="dialog" aria-modal="true" aria-labelledby="promo-confirm-title">
      <div className={`flex items-center justify-between border-b p-4 sm:p-6 ${header}`}><div><p className={`text-[10px] font-bold uppercase tracking-[0.18em] ${eyebrow}`}>{dangerous ? "Protected action" : "Protected action"}</p><h2 id="promo-confirm-title" className="mt-1 text-base font-bold text-[#2D2E30] sm:text-lg">{title}</h2></div><button type="button" onClick={onClose} disabled={saving} className="shrink-0 rounded-lg p-1 text-[#765F55] transition hover:bg-white hover:text-[#2D2E30] disabled:opacity-50" aria-label="Close confirmation"><X size={20} className="sm:h-6 sm:w-6" /></button></div>
      <div className="p-4 sm:p-6"><p className="mb-4 text-sm leading-6 text-[#765F55]">{description}</p>{error && <div role="alert" className="mb-3 rounded-xl border border-red-200 bg-red-50 px-3 py-2.5 text-sm text-red-700">{error}</div>}<label className="mb-2 block text-xs font-bold text-[#2D2E30] sm:text-sm">Confirm admin password</label><input autoFocus type="password" value={password} onChange={(e) => onChange(e.target.value)} onKeyDown={(event) => { if (event.key === "Enter") onConfirm(); }} placeholder="Enter your admin password to confirm" className="w-full rounded-xl border border-[#2D2E30]/15 px-3 py-3 text-sm outline-none focus:border-[#E58C1A] focus:ring-4 focus:ring-[#E58C1A]/10" disabled={saving} autoComplete="current-password" /></div>
      <div className="flex flex-col-reverse gap-2 border-t border-[#2D2E30]/10 bg-[#FFFDF8] p-4 sm:flex-row sm:gap-3 sm:p-6"><button type="button" onClick={onClose} disabled={saving} className="flex-1 rounded-xl border border-[#2D2E30]/15 bg-white px-4 py-2.5 text-sm font-semibold text-[#2D2E30] transition hover:bg-[#FFF4D8] disabled:opacity-50">Cancel</button><button type="button" onClick={onConfirm} disabled={saving} className={`flex-1 rounded-xl px-4 py-2.5 text-sm font-bold text-white transition disabled:cursor-not-allowed disabled:opacity-60 ${dangerous ? "bg-[#A34D45] hover:bg-[#8D4039]" : "bg-[#2D2E30] hover:bg-[#E58C1A]"}`}>{saving ? "Working…" : dangerous ? "Continue" : confirmLabel}</button></div>
    </div>
  </div>;
}
