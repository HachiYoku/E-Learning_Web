...
function PromoCodes() {
  ...
  const payload = () => ({ ...form, discountValue: Number(form.discountValue), fixedAmounts: Object.fromEntries(Object.entries(form.fixedAmounts || {}).filter(([, value]) => value !== "").map(([currency, value]) => [currency, Number(value)])), usageLimit: form.usageLimit ? Number(form.usageLimit) : null, startsAt: used ? form._startsAtRaw : (form.startsAt || null), expiresAt: used ? form._expiresAtRaw : (form.expiresAt || null), applicableCourses: form.applicableCourses || [] });
