              ...
              <div className="mt-5 grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(270px,0.62fr)] lg:items-start">
                <div className={`order-2 grid gap-2 sm:grid-cols-5 sm:gap-3 lg:order-1 ${group.mobileGridClass} ${group.gridClass}`}>
                  {group.vowels.map((vowel) => {
                    ...
                    return <button key={vowel.symbol} type="button" onClick={() => selectVowel({ ...vowel, group })} aria-pressed={isSelected} aria-label={`${vowel.thaiName}, ${group.name}`} className={`min-h-24 rounded-xl border px-2 py-3 text-center transition focus:outline-none focus:ring-4 focus:ring-[#E58C1A]/20 sm:min-h-0 sm:py-4 ${isSelected ? "border-[#C97112] bg-white/80 text-[#2D2E30] shadow-[0_8px_18px_-12px_rgba(80,48,19,0.8)]" : "border-[#2D2E30]/10 bg-white/80 text-[#2D2E30] hover:border-[#E58C1A]/50 hover:bg-[#FFF1D0]"}`}><span className="block text-2xl font-bold leading-none sm:text-3xl">{vowel.symbol}</span><span className="mt-2 block text-xs font-bold text-[#765F55]">{vowel.name}</span></button>;
                  })}
                </div>
