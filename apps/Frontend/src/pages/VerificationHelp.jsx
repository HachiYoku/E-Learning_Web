...
function VerificationHelp() {
  ...
  return <AuthShell eyebrow="Account verification" title={<>Let’s verify your <span className="text-[#E58C1A]">email.</span></>} description="If your link has expired or gone missing, we can send a fresh verification email." footer={<>Ready to sign in? <Link to="/login" className="font-bold text-[#C97112] hover:underline">Go to login</Link><span className="mx-2 text-[#9B867C]">·</span><Link to="/register" className="font-bold text-[#C97112] hover:underline">Use another email</Link></>}>
    <form onSubmit={handleResendVerification} className="space-y-5">
      ...
      <p className="-mt-2 text-sm text-[#765F55]">Entered the wrong address? <Link to="/register" className="font-bold text-[#C97112] hover:underline">Register with another email</Link>.</p>
