import { CalendarClock, Edit3, Layers3, Plus, RefreshCw, Trash2, X } from "lucide-react";
...
function MyFlashcards() {
  ...
  const load = async () => {
    ...
    try { setDecks(await fetchPersonalFlashcardDecks()); } catch (requestError) { setError(requestError.message); } finally { setLoading(false); }
  };
  useEffect(() => { load(); }, []);
  ...
  const saveDeck = async (name) => {
    ...
    try {
      ...
      setDeckModal(null);
    } catch (requestError) { setDeckModal((state) => ({ ...state, error: requestError.message })); } finally { setPending(false); }
  };
  ...
  const removeDeck = async () => {
    ...
    catch (requestError) { setDeleteTarget((state) => ({ ...state, error: requestError.message })); } finally { setPending(false); }
  };

  return <div className="mx-auto max-w-6xl px-4 py-8 sm:px-6 sm:py-10 lg:px-10 lg:py-12">
    <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.22em] text-[#C97112]">Practice</p><h1 className="mt-3 text-3xl font-bold tracking-tight sm:text-4xl">My Flashcards</h1><p className="mt-3 max-w-2xl text-sm leading-relaxed text-[#765F55] sm:text-base">Your private decks for words and phrases you want to remember.</p></div><button type="button" onClick={() => setDeckModal({ deck: null, error: "" })} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#2D2E30] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#E58C1A]"><Plus size={18} />Create Deck</button></header>
...
function DeckCard({ deck, onRename, onDelete }) { return <article className="flex min-h-52 flex-col rounded-3xl border border-[#2D2E30]/10 bg-white p-5 shadow-[0_14px_34px_-28px_rgba(45,46,48,.5)] sm:p-6"><div className="flex items-start justify-between gap-3"><span className="flex h-11 w-11 items-center justify-center rounded-2xl bg-[#E9F4EA] text-[#246B35]"><Layers3 size={21} /></span><span className="rounded-full bg-[#FFF9EA] px-2.5 py-1 text-xs font-bold text-[#765F55]">{deck.cardCount} {deck.cardCount === 1 ? "card" : "cards"}</span></div><h2 className="mt-6 break-words text-xl font-bold text-[#2D2E30]">{deck.name}</h2><p className="mt-2 flex items-center gap-1.5 text-xs text-[#9B867C]"><CalendarClock size={14} />Updated {formatDate(deck.updatedAt)}</p><div className="mt-auto grid grid-cols-3 gap-2 border-t border-[#2D2E30]/10 pt-5"><Link to={`/app/practice/flashcards/mine/${deck._id}`} className="col-span-1 inline-flex min-h-11 items-center justify-center rounded-xl bg-[#2D2E30] px-2 text-sm font-bold text-white transition hover:bg-[#E58C1A]">Manage</Link><button type="button" onClick={onRename} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-[#2D2E30]/15 px-2 text-[#765F55] transition hover:border-[#E58C1A]/40 hover:bg-[#FFF9EA]" aria-label={`Rename ${deck.name}`}><Edit3 size={17} /></button><button type="button" onClick={onDelete} className="inline-flex min-h-11 items-center justify-center rounded-xl border border-red-200 px-2 text-[#A34D45] transition hover:bg-red-50" aria-label={`Delete ${deck.name}`}><Trash2 size={17} /></button></div></article>; }
...
function Empty({ onCreate }) { return <section className="mt-8 rounded-3xl border border-dashed border-[#E58C1A]/35 bg-white px-5 py-14 text-center"><span className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-[#FFF1D0] text-[#C97112]"><Layers3 size={26} /></span><h2 className="mt-5 text-xl font-bold text-[#2D2E30]">No flashcard decks yet</h2><p className="mx-auto mt-2 max-w-sm text-sm leading-6 text-[#765F55]">Create a deck for words you want to remember and practice.</p><button type="button" onClick={onCreate} className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#2D2E30] px-4 py-3 text-sm font-bold text-white transition hover:bg-[#E58C1A]"><Plus size={17} />Create your first deck</button></section>; }
