...
function MyFlashcardDeck() {
  ...
  const saveCard = async (fields) => {
    ...
    try {
      ...
      setCardModal(null);
    } catch (requestError) { setCardModal((state) => ({ ...state, error: requestError.message })); } finally { setPending(false); }
  };
  ...
  const renameDeck = async (name) => {
    ...
    catch (requestError) { setRenameOpen({ error: requestError.message }); } finally { setPending(false); }
  };
  ...
  const removeCard = async () => {
    ...
    catch (requestError) { setDeleteCardTarget((item) => ({ ...item, error: requestError.message })); } finally { setPending(false); }
  };
  ...
  const removeDeck = async () => {
    ...
    catch (requestError) { setDeleteDeckOpen({ error: requestError.message }); } finally { setPending(false); }
  };

  if (loading) return <DeckLoading />;
  if (error || !deck) return <LoadError error={error || "Flashcard deck not found."} retry={load} />;
  ...
  return <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 sm:py-10 lg:px-10 lg:py-12">
    ...
    <header className="mt-5 flex flex-col gap-5 border-b border-[#2D2E30]/10 pb-7 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs font-bold uppercase tracking-[.22em] text-[#C97112]">Private deck</p><h1 className="mt-3 break-words text-3xl font-bold tracking-tight sm:text-4xl">{deck.name}</h1><p className="mt-2 text-sm text-[#765F55]">{deck.cardCount} {deck.cardCount === 1 ? "card" : "cards"}</p></div><div className="grid grid-cols-2 gap-2 sm:flex"><button type="button" onClick={() => setRenameOpen({ error: "" })} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl border border-[#2D2E30]/15 px-4 text-sm font-bold text-[#765F55] hover:border-[#E58C1A]/40 hover:bg-[#FFF9EA]"><Edit3 size={16} />Rename</button><Link to={practicePath} aria-disabled={!cards.length} onClick={(event) => { if (!cards.length) event.preventDefault(); }} className={`inline-flex min-h-11 items-center justify-center gap-2 rounded-xl px-4 text-sm font-bold ${cards.length ? "bg-[#E9F4EA] text-[#246B35] hover:bg-[#D6EFD9]" : "cursor-not-allowed bg-[#F2EFEB] text-[#9B867C]"}`}>Start practice</Link><button type="button" onClick={() => setCardModal({ card: null, error: "" })} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-xl bg-[#2D2E30] px-4 text-sm font-bold text-white hover:bg-[#E58C1A]"><Plus size={17} />Add card</button></div></header>
    ...
    <button type="button" onClick={() => setDeleteDeckOpen({ error: "" })} className="mt-8 inline-flex min-h-11 items-center gap-2 rounded-xl px-3 text-sm font-bold text-[#A34D45] hover:bg-red-50"><Trash2 size={16} />Delete deck</button>
    ...
    {deleteDeckOpen && <ConfirmModal title={`Delete “${deck.name}”?`} detail={`This will permanently delete this deck and all ${deck.cardCount} ${deck.cardCount === 1 ? "flashcard" : "flashcards"} inside it.`} error={deleteDeckOpen.error} pending={pending} confirm="Delete deck" onCancel={() => !pending && setDeleteDeckOpen(false)} onConfirm={removeDeck} />}
  </div>;
}
...
function EmptyCards({ onAdd }) { return <section className="mt-7 rounded-3xl border border-dashed border-[#E58C1A]/35 bg-white px-5 py-12 text-center"><h2 className="text-xl font-bold text-[#2D2E30]">No flashcards in this deck yet</h2><p className="mt-2 text-sm text-[#765F55]">Add the first word or phrase you want to remember.</p><button type="button" onClick={onAdd} className="mt-6 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#2D2E30] px-4 py-3 text-sm font-bold text-white hover:bg-[#E58C1A]"><Plus size={17} />Add card</button></section>; }
