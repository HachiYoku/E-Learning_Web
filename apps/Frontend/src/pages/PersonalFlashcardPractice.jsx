...
const { deckId } = useParams();
  ...
  if (error || !deck) return <FailureState error={error || "Flashcard set not found."} retry={load} deckPath={deckPath} />;
  if (!cards.length) return <EmptyState deck={deck} deckPath={deckPath} />;
  return <div className="mx-auto max-w-4xl px-4 py-8 sm:px-6 sm:py-10 lg:px-10 lg:py-12"><header className="border-b border-[#2D2E30]/10 pb-7"><Link to={deckPath} className="inline-flex min-h-10 items-center gap-2 rounded-xl px-2 text-sm font-bold text-[#765F55] hover:bg-[#FFF1D0] hover:text-[#C97112]"><ArrowLeft size={17} />Back to Flashcards</Link><p className="mt-5 text-xs font-bold uppercase tracking-[.22em] text-[#C97112]">Private practice</p><h1 className="mt-3 break-words text-3xl font-bold tracking-tight sm:text-4xl">{deck.name}</h1><p className="mt-2 text-sm text-[#765F55]">{cards.length} {cards.length === 1 ? "flashcard" : "flashcards"} in this practice session</p></header><FlashcardPracticeSession cards={cards} onExit={() => navigate(deckPath)} exitLabel="Back to Flashcards" /></div>;
}
...
function FailureState({ error, retry, deckPath }) { return <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-10"><Link to={deckPath} className="inline-flex min-h-10 items-center gap-2 rounded-xl px-2 text-sm font-bold text-[#765F55] hover:bg-[#FFF1D0]"><ArrowLeft size={17} />Back to Flashcards</Link><section className="mt-6 rounded-3xl border border-red-100 bg-white px-5 py-12 text-center"><p className="text-sm text-[#765F55]">{error}</p><button type="button" onClick={retry} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#2D2E30] px-4 py-3 text-sm font-bold text-white hover:bg-[#E58C1A]"><RefreshCw size={17} />Try again</button></section></div>; }
function EmptyState({ deck, deckPath }) { return <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-10"><Link to={deckPath} className="inline-flex min-h-10 items-center gap-2 rounded-xl px-2 text-sm font-bold text-[#765F55] hover:bg-[#FFF1D0]"><ArrowLeft size={17} />Back to Flashcards</Link><section className="mt-6 rounded-3xl border border-dashed border-[#E58C1A]/35 bg-white px-5 py-12 text-center"><h1 className="text-2xl font-bold text-[#2D2E30]">{deck.name}</h1><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[#765F55]">Add some flashcards before you start practicing.</p><Link to={deckPath} className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-[#2D2E30] px-4 py-3 text-sm font-bold text-white hover:bg-[#E58C1A]">Add a card</Link></section></div>; }
