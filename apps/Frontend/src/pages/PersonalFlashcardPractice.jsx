...
const { deckId } = useParams();
  ...
  const myFlashcardsPath = "/app/practice/flashcards/mine";
...
function FailureState({ error, retry, backPath }) { return <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-10"><Link to={backPath} className="inline-flex min-h-10 items-center gap-2 rounded-xl px-2 text-sm font-bold text-[#765F55] hover:bg-[#FFF1D0]"><ArrowLeft size={17} />Back to Flashcards</Link><section className="mt-6 rounded-3xl border border-red-100 bg-white px-5 py-12 text-center"><p className="text-sm text-[#765F55]">{error}</p><button type="button" onClick={retry} className="mt-5 inline-flex min-h-11 items-center gap-2 rounded-xl bg-[#2D2E30] px-4 py-3 text-sm font-bold text-white hover:bg-[#E58C1A]"><RefreshCw size={17} />Try again</button></section></div>; }
function EmptyState({ deck, deckPath, backPath }) { return <div className="mx-auto max-w-4xl px-4 py-10 sm:px-6 lg:px-10"><Link to={backPath} className="inline-flex min-h-10 items-center gap-2 rounded-xl px-2 text-sm font-bold text-[#765F55] hover:bg-[#FFF1D0]"><ArrowLeft size={17} />Back to Flashcards</Link><section className="mt-6 rounded-3xl border border-dashed border-[#E58C1A]/35 bg-white px-5 py-12 text-center"><h1 className="text-2xl font-bold text-[#2D2E30]">{deck.name}</h1><p className="mx-auto mt-3 max-w-md text-sm leading-6 text-[#765F55]">Add some flashcards before you start practicing.</p><Link to={deckPath} className="mt-6 inline-flex min-h-11 items-center rounded-xl bg-[#2D2E30] px-4 py-3 text-sm font-bold text-white hover:bg-[#E58C1A]">Add a card</Link></section></div>; }
